...
- **Persistence** ring buffer memstore, durable stream cursors, file-based WAL (`persist_dir`) with CRC32
  Note: `$JS.<name>` PUBLISH appends to a named in-memory stream (v0.5.93). Message bodies stay on the ring; cursors persist under `{persist_dir}/js` when set.
...
- **Durable stream cursors (v0.5.56 / v0.5.87 / v0.5.93)**: opt-in `{dir}/{name}.cursors` ack watermarks; 1–16 hash partitions via `append_key` / `next_part` / `ack_part`; `$JS.<name>` PUBLISH append (D4)
