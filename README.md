...
- **Durable stream cursors (v0.5.56 / v0.5.87 / v0.5.93–95 / v0.5.103–107)**: opt-in `{dir}/{name}.cursors` ack watermarks; last `$JS` payload in `{dir}/js/{name}.last`; history WAL `{dir}/js/{name}.msgs`; `$JS` 1–16 hash partitions (`set_partitions` / `js_partitions` default / `{name}.parts` / payload `append_key` / `consume_part`); `$JS.<name>` PUBLISH + REQUEST-get; `$JS.<name>.<consumer>` consume / ack (D4)
