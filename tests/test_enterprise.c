...
TEST(tls, config_set_fields) {
    ...
    /* F1: backend_secure now returns 1 when OpenSSL is linked. */
...
TEST(tls, session_lifecycle) {
    cmq_tls_config_t *cfg = cmq_tls_config_create();
    /* F1: real OpenSSL backend requires actual cert/key files. Use the
     * test certs generated by openssl CLI; CMQ_TLS_OPENSSL guards the
     * load path. */
    cmq_tls_set_cert(cfg, "/tmp/cmq_test_cert.pem");
    cmq_tls_set_key(cfg, "/tmp/cmq_test_key.pem");
    ...
    /* Real handshake: synchronous on a pipe (no want_read/want_write
     * retries), so a single call should complete or fail. */
    int hs = cmq_tls_handshake(srv);
    ASSERT(hs >= 0);
