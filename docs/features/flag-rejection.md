...
implemented for **BATCH only** (F2, v0.5.41). Prior to the F11 reject, a
...
is set on any opcode other than `CMQ_OP_BATCH`. The server's accept
loop drains the queue and tears down the connection on `pending_error`.

The rule is **fail-closed**: a compressed PUBLISH must not silently
round-trip garbage. BATCH is the only opcode with a decompress path.
...
- `CMQ_FLAG_COMPRESSED (0x01)` — accepted on `CMQ_OP_BATCH` only;
  rejected on PUBLISH/MESSAGE (v0.5.41).
- `CMQ_FLAG_CHECKSUM (0x02)` — accepted; verified in `handle_publish`.
- `COMPRESSED` on non-BATCH opcodes — rejected (F11).
